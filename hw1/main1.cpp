...
int main(){
    ...
    std::ofstream points_file("mesh1/points.txt");
    for(auto& node : nodes){
        points_file << node.x1 << " " << node.x2  << " " << node.x3 << "\n";
    }

    std::ofstream hexas_file("mesh1/hexas.txt");
    ...
    for(int i = 0 ; i < Nt ; i++){
        ...
        if(nodes[i].x1 == x1_ll && nodes[i].x2 == x2_ll && nodes[i].x3 == x3_ll){
        ...
        if(nodes[i].x1 == x1_ll && nodes[i].x2 == x2_ll && nodes[i].x3 == x3_ul){
            nodeLocationsD_map[i].push_back(1); // 1 => X2 displacement specified on this node
        }
    }
    ...
    for(int i = 0 ; i < dirischletIndexes.size() ; i++){
        ...
        if(nodes[nodeD].x1 == x1_ll){
            if(dof == 0){
                dirischletVal(i) = 0.0; //X1 displacements at x1=0 are fixed to 0
            }
        }
        ...
        if(nodes[nodeD].x1 == x1_ll && nodes[nodeD].x2 == x2_ll && nodes[nodeD].x3 == x3_ll){
            if(dof == 1 || dof == 2){
                dirischletVal(i) = 0.0; //X2 and X3 displacements at x1=0 are fixed to 0
            }
        }
        if(nodes[i].x1 == x1_ll && nodes[i].x2 == x2_ll && nodes[i].x3 == x3_ul){
            if(dof == 1){
    ...
    for(unsigned int increment = 0; increment < maxIncrement; increment++){
        ...
        std::string filename = "solutions1/solution_" + std::to_string(increment+1) + ".vtu";
        write_vtu(filename, nodes, elements, u);
        solution_files.push_back(filename);
        solution_timesteps.push_back(increment+1);
    }

    write_pvd("solutions1/final_solution.pvd", solution_files, solution_timesteps);
