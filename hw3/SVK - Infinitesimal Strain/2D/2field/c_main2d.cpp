        ...
        if constexpr (Nne == 4 || Nne == 9){
            if constexpr (Npe == 4){
                if(!bubble){//P dofs at vertices
                    ...
                    switch (node) {
                        case 0: 
                            value = b * lm_xi  * lm_eta;   // (-g, -g)
                            break;
                        case 1: 
                            value = b * lp_xi  * lm_eta;   // (+g, -g)
                            break;
                        case 2: 
                            value = b * lp_xi  * lp_eta;   // (+g, +g)
                            break;
                        case 3: 
                            value = b * lm_xi  * lp_eta;   // (-g, +g)
                            break;
                        default: 
                            throw std::invalid_argument("cannot evaluate basis function value for out of bound local node number");
                    }
...
int main(){
    ...
    Eigen::MatrixXf GglobalT = Eigen::MatrixXf::Zero(Nnp,Nt*Nsd);  // [Nnp x 2*Nnu]  (G^T in notes)
    ...
    for(int e = 0; e < Nel_t ; e++){
        ...
        if constexpr(Nne == 3){
            ...
            for(int q = 0 ; q < qp.points.size() ; q++){
                ...
                float w_q   = qp.weights[q];  // weight already includes 1/2 factor for triangle
                ...
                // ---- K_local: [2*Nne x 2*Nne] ----
                // K^{AB}_{ij} = ∫ 2μ · N^A_{,k} · N^B_{,k} · δᵢⱼ  dV
                // (isotropic, full gradient contraction — volumetric handled by G/M)
                // In Voigt/block form: K^{AB}_{ij} = δᵢⱼ · (∑_k dNA/dxk · dNB/dxk) · 2μ
                for(int A = 0 ; A < Nne ; A++){
                    auto [dNA_dxi1, dNA_dxi2] = basis_gradient<Nne,BfOrder>(A, xi1_q, xi2_q);
                    // physical gradients via Jinv: dN/dx = Jinv^T · dN/dxi
                    ...
                    for(int B = 0 ; B < Nne ; B++){
                        ...
                        // dot product of physical gradients: ∑_k dNA/dxk · dNB/dxk
                        float grad_dot = dNA_dx1*dNB_dx1 + dNA_dx2*dNB_dx2;

                        // K^{AB} is a 2x2 block = 2μ · grad_dot · I_{2x2}
                        // (δᵢⱼ contribution only — volumetric 2μ δᵢⱼ εₖₖ term moves to G)
                ...
                // ---- GlocalT: [Npe x 2*Nne] ----
                // GlocalT(A,B_i) = ∫ N^A_p · N^B_{u,i} · det(J) dV^ξ
                // where B_i means: column = 2*B + i  (displacement dof i of node B)
                for(int A = 0 ; A < Npe ; A++){
                ...
                // ---- Mlocal: [Npe x Npe] ----
                // Mlocal(A,B) = ∫ N^A_p · N^B_p / λ · det(J) dV^ξ
                for(int A = 0 ; A < Npe ; A++){
                    float NA_p = basis_functionP<Nne,Npe,BfOrderP>(bubble, A, xi1_q, xi2_q);
                    for(int B = 0 ; B < Npe ; B++){
        ...
        if constexpr(Nne == 4 || Nne == 9){
            // 2x2 tensor-product Gauss quadrature for quads
            auto qp = gauss_legendre<Nne>(quadRule);
    ...
    // G^T sub-blocks: rows=all pressure DOFs, cols split by free/fixed u-dofs
    // We need all rows of GglobalT (no BC on pressure), cols split
    ...
    Eigen::VectorXf F2 =     - GD  * dirischletVal;  // [Nnp x 1]

    // Factorise KUU once — reused in both steps
    ...
    // KUU^{-1} · GU^T  : [Nu_free x Nnp]
    Eigen::MatrixXf GUT = GU.transpose();                                // [Nu_free x Nnp]
    Eigen::MatrixXf KinvGUT = llt_KUU.solve(GUT);                       // [Nu_free x Nnp]

    // KUU^{-1} · F1 : [Nu_free x 1]
    Eigen::VectorXf KinvF1 = llt_KUU.solve(F1);                         // [Nu_free x 1]

    // Schur complement matrix S = GU · KinvGUT - M  : [Nnp x Nnp]
    Eigen::MatrixXf S = GU * KinvGUT - Mglobal;                         // [Nnp x Nnp]

    // Schur complement RHS = GU · KinvF1 - F2       : [Nnp x 1]
    Eigen::VectorXf S_rhs = GU * KinvF1 - F2;

    // Step 1: Solve S · alpha = S_rhs for pressure DOFs
    Eigen::VectorXf alpha = S.lu().solve(S_rhs);                         // [Nnp x 1]

    // Step 2: Solve for free displacement DOFs
    Eigen::VectorXf DU = KinvF1 - KinvGUT * alpha;                      // [Nu_free x 1]
