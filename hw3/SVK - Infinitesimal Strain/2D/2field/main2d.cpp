...
int main(){
    ...
    constexpr unsigned int BfOrder = 1; //Order of basis functions 
    constexpr unsigned int Nne = 4; //number of nodes in an element - 8 for hexahedral element
    unsigned int quadRule = 1; //quadrature rule for numerical integration

    constexpr unsigned int BfOrderP = 1;
    constexpr unsigned int Npe = 4;
    unsigned int quadRuleP = 1;
    bool bubble = false; //false = P dofs at the u dofs (vertices)
    ...
    Eigen::MatrixXf GglobalT = Eigen::MatrixXf::Zero(fill dimensions here);
    Eigen::MatrixXf Mglobal = Eigen::MatrixXf::Zero(fill dimensions here);
    ...
    for(int e = 0; e < Nel_t ; e++){
        ...
        for(int A = 0; A < Npe; A++){
            ...
            {
                int Bglobal = elements[e].node[B];
                //GglobalT assembly code goes here
            }
        }
        ...
        for(int A = 0; A < Npe; A++){
            ...
            for(int B = 0; B < Npe ; B++)
            {
                ...
                //MglobalT assembly code goes here
            }
        }
    ...
    //Solve for unknown displacements using the formulation given in the handwritten formulation pdf
